...
class Kayttoliittyma:
    ...
    def kaynnista(self):
        ...
        summa_painike = ttk.Button(
            master=self._root,
            text="Summa",
            command=lambda: self._komennot[Komento.SUMMA].suorita()
        )

        erotus_painike = ttk.Button(
            master=self._root,
            text="Erotus",
            command=lambda: self._komennot[Komento.EROTUS].execute(self._lue_syote())
        )
...
class Summa():
    ...
    def suorita(self):
        self._sovellus.plus(self.syote)
...
class Nollaus():
    ...
    def suorita(self):
        self._sovellus.nollaa(self.syote)
...
class Laskin:
    def _suorita_komento(self, komento):
        ...
        command = self._commands.get(komento)
        if command:
            command.execute(arvo)
